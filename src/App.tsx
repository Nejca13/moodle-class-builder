...
function validate(
  ...
  for (const field of template.fields) {
    ...
    if (text && field.type === 'image' && !sanitizeImageSrc(text)) {
      errors[field.id] =
        'La imagen no es válida (usá una URL o un archivo de imagen).'
    }
  }
...
export default function App() {
  ...
  const [values, setValues] = useState<TemplateValues>(() => {
    const last = getLastTemplateId()
    return last && getTemplateById(last)
      ? (getDraft(last) ?? emptyValues())
      : emptyValues()
  })
  ...
  const handleCopyHtml = () => {
    runIfValid(async () => {
      ...
      setStatus(
        ok ? 'HTML copiado correctamente.' : 'No se pudo copiar el HTML.',
      )
    })
  }
  ...
  const handleCopyText = () => {
    runIfValid(async () => {
      ...
      setStatus(
        ok ? 'Texto copiado correctamente.' : 'No se pudo copiar el texto.',
      )
    })
  }
  ...
  return (
    <div className='min-h-screen text-slate-900 flex flex-col selection:bg-indigo-500 selection:text-white'>
      ...
      <footer className='max-w-6xl mx-auto px-4 py-8 text-center text-xs text-slate-500 font-medium space-y-2'>
        <p>
          Moodle Class Builder · Generador de contenido educativo compatible con
          Moodle TinyMCE
        </p>
